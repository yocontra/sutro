...
const parseIncludes = include => {
  ...
  return inp.map((i, idx) => {
    ...
    if (typeof i.resource !== 'string') throw new Error(`Invalid include resource: ${idx}`);

    // tack on to keep it minimal
    const out = { resource: i.resource };
